...
def half_list(list_in: List, half: int) -> List:
    ...
    :return: A list.
    """
    # what's the better way to do this? keep fishing.
    mid = math.ceil(len(list_in) / 2)
    length = int(len(list_in))
...
def remove_odds(list_in: List[int]) -> List[int]:
    ...
    :return: None
    """
    #this is only good if we know the list alternates between even & odd numbers..
    for num in list_in:
        if num % 2 != 0:
            list_in.remove(num)
    return list_in
...
def remove_evens(list_in: List[int]) -> List[int]:
    """
